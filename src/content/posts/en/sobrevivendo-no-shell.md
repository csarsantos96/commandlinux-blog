...
sourceHash: 3bda1b11436d68c1d1a05a3977f3673bddb6260442654d1b0b1fc66f0be11f8f
---
These are my notebook notes while studying Linux in depth — part of the **Linux for Cloud Native** course by LINUXtips, within the PICK 2026 track. I've cleaned everything up here because writing is the best way to consolidate knowledge, and because one day this will become a quick reference when I can't remember a flag.
...
In Linux, there's a **single root**: `/`. Everything starts from it. No matter how many disks, partitions, or devices you have — all will be mounted at some point within this tree. No `C:`, `D:`, `E:`.
...
| `/home` | Personal directories for users. If your user is `devops`, it will be `/home/devops`. The `~` shortcut represents your home directory. |
| `/etc` | Where system configuration resides: `/etc/ssh/sshd_config`, `/etc/hostname`, `/etc/passwd` |
...
| `/dev` | Contains device files. Here you'll also find the famous `/dev/null`, Linux's black hole. |
| `/mnt` and `/media` | Mount points for external devices |
| `/opt` | Third-party software, installed outside the package manager |

> **FHS Detail:** The fact that `/bin` and `/sbin` are symlinks pointing into `/usr` is a result of the *usr-merge*, adopted by most distros in the last decade. Run `ls -l /bin` on a recent Fedora or Ubuntu and you'll see the arrow.
...
The `--color=auto` detail is more interesting than it seems: if you do `ls | grep something`, the output doesn't go to the terminal, it goes to the pipe — and `ls` automatically turns off colors. That's why ANSI escape codes don't leak into your `grep` output.
...
> ⚠️ **Use `rm -rf` with extreme caution.** It doesn't ask for confirmation, there's no trash bin, and no undo. Before running with a glob (`*`), replace `rm` with `ls` and check exactly what would be deleted.
...
Mnemonic rule: **c**reate, e**x**tract, lis**t** — always accompanied by `-f` (file). `-z` is for gzip; use `-j` for bzip2 and `-J` for xz.
...
The `-i` option of `ls` shows the inode number — if it's the same for both, it's the same physical file with two names.
...
`less` opens the file in paginated mode, without dumping everything on the screen at once. You navigate calmly:
...
| `space` | Advances a full page |
| `b` | Goes back a full page |
| `q` | Exits |

`less` is the **default pager** for many commands. When you run `man ssh` to read the SSH manual, the content opens in `less`. The same shortcuts work inside. Knowing how to navigate in `less` means knowing how to navigate all Linux documentation.
...
Combine with `grep` to filter only what interests you:

```bash
tail -f /var/log/auth.log | grep "Failed password"
```

Now you see only failed login attempts, in real-time. Perfect for detecting SSH brute force attacks, for example.
...
The `wc` (*word count*) is a simple command that answers quick questions about a file's content:
...
Lines with `-` were removed and lines with `+` were added. Anyone who has used `git diff` will recognize the format immediately — because it is literally the same.
...
| `df -i` | Shows the usage of filesystem **inodes** |
| `df -T` | Shows disk usage including the filesystem type |
| `df -Th` | Human-readable format **and** FS type — the most useful in day-to-day work |

### What is an inode?

An inode is a kind of **"internal record"** that Linux uses to control files and directories. Each file uses 1 inode.

That's why `df -i` matters: it shows if the system still has capacity to create new files and directories — not just if there are free GBs. You can have 200 GB remaining and still receive `No space left on device` because the inodes ran out. It's a classic on servers full of small files, like session caches.
...
It shows kernel, process, memory, CPU, network information, etc. It's like a Linux diagnostic panel — each running process has a `/proc/<PID>` directory with everything about it.
...
If I had to choose the five commands that most changed my day-to-day life from the list above, they would be: `tail -f | grep` (real-time investigation), `less` (navigation in any documentation), `diff -u` (before any production change), `df -Th` (the first command when something breaks), and `ls -lha` (pure reflex).
...
- [GNU Coreutils Manual](https://www.gnu.org/software/coreutils/manual/coreutils.html) — the official manual for practically all commands mentioned here (`ls`, `cp`, `mv`, `rm`, `cat`, `head`, `tail`, `wc`, `df`, `sort`).
